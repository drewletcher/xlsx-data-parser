...
export default class XlsxDataParser extends EventEmitter {
  ...
  async parse() {
    ...
    try {
      if (!this.worksheet) {
        ...
        if (typeof this.options.url === "string" && !this.options.url.toLowerCase().startsWith("http")) {
          // read local file
          // console.debug("load workbook " + this.options.url);
          let data = fs.readFileSync(this.options.url);
          this.workbook = XLSX.read(data, this.options.xlsx);
  ...
  pause() {
    // console.debug(`parser pause ${this.pos} ${this.count}`);
    this.paused = true;
  }

  resume() {
    // console.debug(`parser resume ${this.pos} ${this.count}`);
    this.paused = false;
    this.parseCells();
  }

  cancel() {
    // console.debug(`parser cancel ${this.pos} ${this.count}`);
    this.cancelled = true;
  }
   ...
   */
  async parseCells() {
    if (this.parsing || this.pos >= this.len)
      return;
    this.parsing = true;
    ...
    for (; this.pos < this.len; this.pos++) {
      ...
      if (a1_address[ 0 ] === '!') {
        if (this.options.raw)
          await this.output(this.entries[ this.pos ]);
        continue;
      }
   ...
   */
  async output(row) {
    // console.debug(`parser output ${this.pos} ${this.count}`);

    if (this.listenerCount("data") > 0) {
      let r = this.emit("data", row);
    }
    else
      this.rows.push(row);

    this.count++;
    // console.debug(`parser output done ${this.pos} ${this.count}`);
  }
