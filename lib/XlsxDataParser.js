    ...
    try {
      if (!this.worksheet) {
        ...
        if (typeof this.options.url === "string" && !this.options.url.startsWith("http")) {
          ...
          // make http request
          let httpOptions = Object.assign({}, this.options.http, { responseType: "stream" });
          let rs = await httpRequest(this.options.url, httpOptions);
          if (rs.statusCode !== 200){
            this.emit(new Error("error", "workbook not found: " + this.options.url));
            this.cancelled = true;
            return
          }
        ...
        if (!this.worksheet) {
          this.emit(new Error("error", "sheet not found: " + sheetName));
          this.cancelled = true;
          return
        }
      }
